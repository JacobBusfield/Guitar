import XGuitar from './Guitar.ts'



var YGuitar = new XGuitar()
...
GuitarString.prototype.pluck = function (startTime, velocity, tab) {
	...
	var options = YGuitar.getControlsValues();
...
function asmFunctions(stdlib, foreign, heapBuffer) {
	...
	function renderKarplusStrong(
		...
		*/
		...
		for (targetIndex = 0;
			(targetIndex | 0) < (sampleCount | 0); targetIndex = (targetIndex + 1) | 0) {
			...
			if ((targetIndex | 0) < (periodSamples | 0)) {
				...
				curInputSample = +lowPass(curInputSample, noiseSample,
					pluckDampingCoefficient);
